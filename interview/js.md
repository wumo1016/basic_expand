...
- js 是一门单线程语言, 因为它是运行在浏览器的渲染主线程中, 而渲染主线程只有一个
- 而渲染主线程的工作有很多, 例如: 渲染页面、执行 js 等
- 如果使用同步的方式, 就极有可能造成阻塞, 从而导致消息队列中的很多其他任务无法执行
- 这样一来，一方面会导致繁忙的主线程白白的消耗时间, 另一方面会导致页面无法更新, 给用户造成卡死的现象
- 所以浏览器采用异步的方式来避免。具体做法是方某些任务发生时，比如：计时器、网路、事件监听等
- 主线程将任务交给其他任务去处理，自身立即结束此任务的执行，转而执行后续任务
- 当其他线程完成时，将事先传递的回调包装成任务，加入到消息队列的末尾，等待主线程调度执行
- 在这种模式下，浏览器永不阻塞，从而最大限度的保证了单线程的流畅运行

## 如何实现防篡改水印

- 监听水印元素的父元素，子元素被删掉时或是属性变化时重新添加进去
