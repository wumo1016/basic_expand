...
- 具体就是创建一个继承数组原型对象的新对象，然后重写那七个方法，内部再调用原生的方法
- 然后将 data 中数组的原型对象 `__proto__` 指向这个新对象
- vue2 中通过索引改变数据和长度的变化是无法监控到的，可以使用 Vue.set 去修改；但 vue3 中由于使用了 proxy API，所以天然支持
...
- v-for 的优先级更高(vue3 中 v-if 优先级更高)
- 无论条件是否成立 都会循环指定次数
- 可以将指定数据先筛选出来 再循环
- 它们在编译阶段就已经完成了核心逻辑了
