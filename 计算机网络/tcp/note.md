...
- 如何确定一个 TCP 连接
  - 通过 TCP 四元组确定
    ...
    - 目标端口号: 在 TCP 头中
- 其他
  - 客户端和服务端都可以主动断开连接, 通常先关闭的一方称为主动方, 后关闭的一方称为被动方
  - 关闭连接的方式通常由两种: FIN 报文关闭和 RST 报文关闭
...
- 糊涂窗口综合症
  ...
  - 解决
    - 接收方(接受方不通知小窗口)
      - 通常的策略是当可用窗口大小 < min(MSS, 窗口大小 / 2), 一般就是小于窗口大小一半的时候, 就向发送方通告窗口为 0
    - 发送方
...
- 拥塞窗口(cwnd)
  ...
  - 介绍: 发送方维护的一个状态变量, 它会根据网路的拥塞程度动态变化
  - 发送窗口: 发送窗口 = min(接收窗口, 拥塞窗口)
...
- 拥塞控制的四个算法
  ...
  - 拥塞避免
    - 每当收到一个 ACK, cwnd 就增加 1/cwnd(在一个往返时延内 cwnd 的值是固定的, 但是可能收到多个 ACK)
    - 其实就是将原本慢启动的指数级增长变成了线性增长, 但还是增长阶段
    - 就这样一直增长, 网络慢慢拥塞, 就会丢包, 触发重传机制, 也就进入了拥塞发生算法
  - 拥塞发生
    - 超时重传触发
      - ssthresh 设置为 cwnd / 2
      - cwnd 重置为初始化值
...
- 分类
  - TCP 是面向字节流协议
    - 消息会被操作系统拆成多个 TCP 报文
  - UDP 是面向报文协议
    - 操作系统不会对用户消息进行拆分, 读取一个 UDP 报文就是能读取到完整的用户消息

## 粘包

- 介绍: 当两个消息被分到一个 TCP 报文中, 就是常说的粘包问题(这时如果接收方不知道消息边界的话, 就无法读出有效的消息)
- 三种分包方式
  - 固定长度的消息
    - 当接收方接满指定长度的字节时, 就认为这是一个有效的消息
    - 不灵活, 所以不常用
  - 特殊字符作为边界
    - 在两个用户消息之间插入一个特殊字符, 当读到这个特殊字符的时候, 就认为这是一个完成的消息
    - 如果消息中也有这个特殊字符. 就需要对这个特殊字符进行转义
  - 自定义消息结构
    - 消息由包头和数据组成
    - 包头大小固定
    - 包头里有个字段说明后面的数据有多大
...
## 拔掉网线后, 原来的 TCP 连接还在不在

- 拔掉网线后, 还有没有数据传输
  - 有数据传输
    - 如果服务端发送了数据报文, 且在服务端重传次数到达最大值之前, 就插回了网线, 那么就像什么事都没发生一样
    - 如果插回网线前服务端已经达到重传次数断开了连接, 客户端再次发送数据, 服务端就会回复 RST 报文, 双方都断开这个连接
  - 无数据传输
    - 如果都没有开启 keep-alive 机制, 那么客户端和服务端的 TCP 连接将会一直存在
    - 如果都开启了, 在服务端探测期间, 客户端还是没有网, 那就会断开连接

## HTTPS 中的 TLS 和 TCP 能同时握手吗

- HTTPS 是先进行 TCP 的三次握手, 再进行的 TLS1.2 四次握手
- HTTPS 中的 TLS 和 TCP 能同时握手, 必须同时满足以下两个条件
  - 客户端和服务端都同时开启了 TCP Fast Open 功能, 且 TLS 版本是 1.3
  - 客户端和服务端已经完成过一次通信
...
- HTTP(HTTP 长连接)
  - 由应用程序实现
  - 在第一个 HTTP 请求完成后, 先不断开 TCP 连接, 后续继续使用此连接
- TCP(TCP 保活机制)
  - 由内核实现
  - 当客户端和服务端长时间没有进行交互时, 内核会发送探测报文, 来决定是否关闭该连接
